...
export function SearchResultCard({
  ...
  return (
    <Card className="overflow-hidden w-64 flex-shrink-0 bg-card text-card-foreground shadow-lg cursor-pointer" onClick={handleStreamClick}>
      <CardContent className="p-3 flex flex-col h-full">
        <div className="relative flex items-center justify-center h-80 bg-muted rounded-md mb-3 overflow-hidden">
          {movieInfo?.poster && movieInfo.poster !== "N/A" ? (
            <Image src={movieInfo.poster} alt={movieInfo.title} fill className="object-cover" />
          ) : (
            <Film className="h-20 w-20 text-muted-foreground" />
          )}
        </div>
        ...
        <div className="space-y-2">
          <h3 className="font-semibold text-lg line-clamp-2 text-foreground">{movieInfo?.title || name}</h3>
